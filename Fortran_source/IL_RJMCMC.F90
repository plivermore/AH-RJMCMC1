...
CHARACTER(500) :: ARG, Data_file_name, Header, WRITE_MODEL_FILE_NAME, inputline, Outputs_directory
INTEGER ::  NARG
...
INTEGER :: I, K, BURN_IN, NSAMPLE, K_INIT, K_MAX, K_MIN, K_MAX_ARRAYBOUND, discretise_size, show, thin, num,j, &
           NUM_DATA, IOS, K_MAX_ARRAY_BOUND, ind, NBINS, I_MODEL, FREQ_WRITE_MODELS
REAL( KIND = 8) :: I_MAX, I_MIN, sigma_move, sigma_change_value, sigma_birth, int_j,&
                   X_MIN, X_MAX, age_frac

INTEGER, ALLOCATABLE :: SEED(:)
INTEGER :: stratification(1:MAX_DATA), LIKELIHOOD_CHOICE

CHARACTER(100), Allocatable :: LINE_READ(:)

INTEGER ::  age_col, d_age_col, F_col, dF_col, Strat_col
REAL( KIND = 8) :: age(1:MAX_DATA), delta_age(1:MAX_DATA),  intensity(1:MAX_DATA), delta_intensity(1:MAX_DATA), credible
...
IF(Strat_col > 0) THEN
PRINT*, 'The integrated-likelihood code cannot be run with stratified data'
STOP
ENDIF
...
NUM=ceiling((nsample-burn_in)*0.025/thin) ! number of collected samples for credible intervals
...
ALLOCATE( RETURN_INFO%AV(1:discretise_size), RETURN_INFO%BEST(1:discretise_size),  &
          ...
          RETURN_INFO%change_points(nsample * k_max_array_bound), RETURN_INFO%n_changepoint_hist(K_MIN : K_MAX), &
          RETURN_INFO%convergence(nsample)   )
...
PRINT*, 'Code can only be run with uniformly distributed ages'
STOP
ENDIF
...
! choose the integrated likelihood
LIKELIHOOD_CHOICE = 1

! copy input file
call system('cp '//TRIM(ARG)//' '//TRIM(Outputs_directory)//'/input_file')

CALL RJMCMC(burn_in, NUM_DATA, age(1:NUM_DATA), delta_age(1:NUM_DATA), intensity(1:NUM_DATA), delta_intensity(1:NUM_DATA), NSAMPLE, I_MIN, I_MAX, X_MIN, X_MAX, K_MIN, K_MAX, SIGMA_MOVE, sigma_change_value, sigma_birth, discretise_size, SHOW, THIN, NBINS, RETURN_INFO, CALC_CREDIBLE, credible, FREQ_WRITE_MODELS, WRITE_MODEL_FILE_NAME, LIKELIHOOD_CHOICE, Outputs_directory)
